...
1. Run the following commands to extract the results for the LS and FB strategies. There are different programs to extract the results for single- and multiple-fault versions. You must pass the path to the root folder that contains the report files (e.g., /home/user/experiments/).
```bash
...
The main results of our experiments for single-fault versions are available [here](https://drive.google.com/open?id=0B2Vc3U9nBKO_U0c4UGcxQVBFVTA). For multiple-fault versions are available [here](https://www.dropbox.com/s/i5gyhjc2tf15fmb/road2fault-results-mf.ods?dl=0).
The csv files and logs generated by the calculator are also available for [single](https://drive.google.com/open?id=0B2Vc3U9nBKO_Q0w4SXpUR0hrRDg) and [multiple](https://www.dropbox.com/s/0glgos94fskko6l/ch-icd-results-logs-mult.tar.7z?dl=0) faults.
