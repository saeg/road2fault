...
public class InspectionStrategy {
	...
	/*
	 * This procedure was used in the ICD paper 2015, looking all methods with equal name
	 * */
	public static void calculateMcpPerformance(XmlTestCriteria criteria, XmlMcpTestCriteria mcpCriteria, double delta){
		...
		double faultScore=-1;
		logFileList.add("");
		logFileList.add(">>>>>>>>>>>>>> MCP - Delta "+delta+"  <<<<<<<<<<<<<<<<");
		logFileList.add("");
		...
		for(McpMethod mcpMethod : mcpCriteria.getRoadmap()){
			double deltaScore = mcpMethod.getScore()-(mcpMethod.getScore()*(delta/100));
	...
	public static void calculateMethodHitSpectraPerformance(XmlTestCriteria criteria){
		...
		double faultScore=-1;
		logFileList.add("");
		logFileList.add(">>>>>>>>>>>>>> Method hit spectra  - counting all method and blocks until the fault score  <<<<<<<<<<<<<<<<");
		logFileList.add("");
		List<XmlMethod> methodList = dataParser.createCodeHierarchyMethodList(criteria);
		Collections.sort(methodList);
		for(XmlMethod method : methodList){
	...
	public static void calculateMethodHitSpectraPerformanceForBlocksAndDelta(XmlTestCriteria criteria, double delta){
		...
		logFileList.add("");
		logFileList.add(">>>>>>>>>>>>>> Method hit spectra  - Delta "+delta+" - Less blocks first  <<<<<<<<<<<<<<<<");
		logFileList.add("");
		List<XmlMethod> methodList = dataParser.createCodeHierarchyMethodList(criteria);
		Collections.sort(methodList);
		for(XmlMethod method : methodList){
			double deltaScore = method.getScore()-(method.getScore()*(delta/100));
